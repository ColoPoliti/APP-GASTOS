  ...
  return (
    <div className="space-y-3">
      {gastos.map((gasto) => {
        ...
        return (
          <div key={gasto.id} className="dark:bg-slate-900/20 border border-slate-900 p-4 rounded-xl flex justify-between items-center transition-all hover:border-indigo-500/50">
            <div>
              <span className="text-[14px] px-2 py-0.5 rounded-md uppercase font-bold border" 
            ...
            <div className="flex items-center gap-4">
              ...
              {esPropietario ? (
                <div className="flex gap-2">
                  <button onClick={() => onEditar(gasto)} className="text-slate-500 hover:text-indigo-400"><FaPen /></button>
                  <button onClick={() => onEliminar(gasto)} className="text-slate-500 hover:text-rose-500"><FaTrash /></button>
                </div>
              ) : (
