...
export default function ResumenDeudas({ gastos, transferencias = [], hogarId, sesionId, miembros, onTransferenciaGuardada }) {
    const [perfiles, setPerfiles] = useState([]);
    
    // Si no te pasan los miembros por props, usamos el estado local de perfiles
    const listaMiembros = miembros && miembros.length > 0 ? miembros : perfiles;
    
    const mensajesDeuda = calcularDeudas(gastos, transferencias);

    // Cargamos los perfiles directamente desde Supabase si no vienen por props
    useEffect(() => {
    ...
    const handleCobrarPorWhatsApp = (mensajeDeudaTexto) => {
        ...
        const deudorEncontrado = listaMiembros.find(p =>
        ...
        const mensaje = `Hola *${deudorEncontrado.nombre || nombreDeudor}*, paso a recordarte: "${mensajeDeudaTexto}". ¡Cuando puedas avísame! 💸`;
                                    ...
                                    <div key={index} className="mb-4">
                                        {estaSaldado ? (
                                            ...
                                            (() => {
                                                ...
                                                const monto = resto[0];
                                                const acreedor = resto[1]?.trim();

                                                const perfilAcreedor = listaMiembros.find(p => 
                                                    p.nombre?.trim().toLowerCase() === acreedor?.toLowerCase() ||
                                                    p.email?.toLowerCase().includes(acreedor?.toLowerCase())
                                                );
                                                ...
                                                return (
                                                    <div className="dark:bg-slate-900/80 bg-white shadow border dark:border-slate-800 border-slate-100 p-4 sm:p-5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 rounded-xl">
                                                        ...
                                                        <div className="flex items-center justify-between sm:justify-end w-full sm:w-auto space-x-3 flex-shrink-0 pt-2 sm:pt-0 border-t sm:border-t-0 border-slate-800">
                                                            ...
                                                            {/* Renderizamos el formulario solo si tenemos miembros listos */}
                                                            {listaMiembros.length > 0 && (
                                                                <TransferenciaForm
                                                                    hogarId={hogarId}
                                                                    sesionId={sesionId}
                                                                    miembros={listaMiembros}
                                                                    destinatarioPreseleccionado={perfilAcreedor?.id}
                                                                    onGuardar={() => {
                                                                        if (onTransferenciaGuardada) onTransferenciaGuardada();
                                                                    }}
                                                                />
                                                            )}
